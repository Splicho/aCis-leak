...
public class Cubic
{
	...
	private Creature pickEnemyTarget()
	{
		...
		else if (ownerTarget instanceof Creature && ownerTarget != _owner.getSummon() && ownerTarget != _owner)
		{
			...
			{
				if (((Attackable) ownerTarget).getAI().getAggroList().get(_owner) != null)
					target = (Creature) ownerTarget;
				else if (_owner.getSummon() != null && ((Attackable) ownerTarget).getAI().getAggroList().get(_owner.getSummon()) != null)
					target = (Creature) ownerTarget;
			}
