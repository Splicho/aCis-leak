...
public abstract class Playable extends Creature
{
	 ...
	 * @param process : The {@link String} of process triggering this action, used by logs.
	 ...
	 */
	public abstract void addItem(String process, ItemInstance item, WorldObject reference, boolean sendMessage);
	...
	public boolean testCursesOnAttack(Npc npc, int npcId)
	{
		...
		if (getStatus().getLevel() - npc.getStatus().getLevel() > 8)
		{
			...
			if (getFirstEffect(curse) == null)
			{
				...
				((Attackable) npc).getAI().getAggroList().stopHate(this);
				return true;
			}
		}
			...
			for (final Creature target : targets)
			{
				...
				if (getStatus().getLevel() - target.getStatus().getLevel() > 8)
				{
					...
					if (getFirstEffect(curse) == null)
					{
						...
						((Attackable) target).getAI().getAggroList().stopHate(this);
						return true;
					}
				}
			}
	...
	public ItemInstance transferItem(String process, int objectId, int amount, Playable source, Playable target)
	{
		final ItemInstance newItem = getInventory().transferItem(process, objectId, amount, target, this);
		if (newItem == null)
			return null;
	...
	public void moveToBoatEntrance(Point2D point, Boat boat)
	{
		...
		if (this instanceof Player)
			((Player) this).getBoatInfo().setCanBoard(true);
