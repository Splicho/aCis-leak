...
public class Q620_FourGoblets extends ScheduledQuest
{
	...
	protected void onStart()
	{
		...
		startQuestTimerAtFixedRate("passed", null, null, PASSED_PERIOD, PASSED_PERIOD);
	}
	...
	public void onAttacked(Npc npc, Creature attacker, int damage, L2Skill skill)
	{
		...
		{
			if (Rnd.get(100) < 33 && attacker == ((Attackable) npc).getAI().getAggroList().getMostHatedCreature())
			{
				npc.getAI().addCastDesire(npc, STATUE_OF_PROTECTION_EFFECT, 1, 1000000);
			}
		}
	}
	...
	public void onClanAttacked(Attackable caller, Attackable called, Creature attacker, int damage, L2Skill skill)
	{
		...
		if (npcId == STATUE_OF_PROTECTION)
		{
			if (Rnd.get(100) < 33)
			{
				called.getAI().addCastDesire(called, STATUE_OF_PROTECTION_EFFECT, 1, 1000000);
			}
		}
	}
	...
	public void onCreated(Npc npc)
	{
		...
		else if (VICTIM_SPAWNS.containsKey(npcId))
		{
			...
			startQuestTimer("survived", npc, null, 300000);
			startQuestTimerAtFixedRate("action", npc, null, 5000, 5000);
	...
	public void onMyDying(Npc npc, Creature killer)
	{
		...
		if (HALISHA_GOBLETS.containsKey(npcId))
		{
			...
			if (count > 0 && npc.getSpawn() instanceof MultiSpawn)
			{
				...
				SpawnManager.getInstance().spawnEventNpcs(event, false);
	...
	public void onSeeCreature(Npc npc, Creature creature)
	{
		...
		if (VICTIM_SPAWNS.containsKey(npcId))
		{
			if (creature instanceof Player)
			{
				...
				{
					npc.getAI().addCastDesire(creature, Rnd.get(VICTIM_BUFFS), 1, 1000000);
				}
			...
			else if (creature instanceof Attackable)
			{
				...
				if (Rnd.get(100) < 80 && ArraysUtil.contains(VICTIM_ATTACKERS, attacker.getNpcId()))
				{
					if (Rnd.get(100) < 80)
					{
						attacker.getAI().getAggroList().addDamageHate(npc, 0, 300);
					}
					else
					{
						attacker.getAI().getAggroList().clear();
						attacker.getAI().getAggroList().addDamageHate(npc, 0, 1000);
					}
				}
			}
		}
	...
	private final void startStatueEffect(Npc npc)
	{
		...
		npc.broadcastPacket(new MagicSkillUse(npc, CHARM_OF_CORNER_EFFECT, 1, 500, 1000));
		startQuestTimerAtFixedRate("statue", npc, null, 3000, 3000);
		npc.setScriptValue(1);
	}
