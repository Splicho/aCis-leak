		...
		forEach(doc, "list", listNode -> forEach(listNode, "castle", castleNode ->
		{
			...
			if (castle != null)
			{
				...
				forEach(castleNode, "controlTowers", controlTowersNode -> forEach(controlTowersNode, "tower", towerNode ->
				{
					...
					final Location location = parseLocation(towerAttrs, "loc");
					
					castle.getControlTowers().add(new TowerSpawnLocation(13002, location));
				}));
				forEach(castleNode, "flameTowers", flameTowersNode -> forEach(flameTowersNode, "tower", towerNode ->
				{
					final NamedNodeMap towerAttrs = towerNode.getAttributes();
					final Location location = parseLocation(towerAttrs, "loc");
					final List<Integer> zoneIds = Arrays.stream(parseString(towerAttrs, "zones").split(";")).map(Integer::parseInt).collect(Collectors.toList());
					
					castle.getFlameTowers().add(new TowerSpawnLocation(13004, location, zoneIds));
				}));
